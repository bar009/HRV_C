...
`Info.plist`, entitlements). בלי זה שום דבר לא נבנה. *כרגע: Swift מותקן ורץ על Windows;
קומפילציה מקומית חסומה על חוסר MSVC/SDK — פרטים ופתרון ב-Track G.*

**S1 — פורט הליבה ([Track A](track-A-core-port.md)).**
פורט 1:1 של `hrv_core/` ל-`Sources/HRVCore` + `HRVCoreTests`. זו אבן-הדרך של האימות: אותם
test vectors, אותם מספרים כמו `tests/` בפייתון. הליבה תלויה ב-Foundation בלבד ולכן מתקמפלת
מחוץ ל-Mac.
