...
> **מסמכי עבודה משלימים**
> - [דרישות המוצר המחייבות ל-V1](PRODUCT_REQUIREMENTS_HE.md) — דרישות מוצר, פרטיות, אבטחה, App Store, בדיקות ושיווק שיש לסגור לקראת ההשקה.
> - [דף עזר למדיטציות נשימה](reference/BREATHING_MEDITATIONS_HE.md) — חומר עזר לפיתוח תוכן וחוויית המדיטציות; אינו רשימת דרישות להשקה.
...
|---|------|--------|--------|----------------|
| 1 | **HealthKit permissions מדויקות** | ✅ | 🟩 | סט הבקשה = **SDNN + workouts + sleepAnalysis + restingHeartRate** — כל טיפוס אכן נקרא בקוד (SDNN לזיהוי; אימונים/שינה לסינון מנוחה; restingHeartRate + אימונים/שינה גם להקשר עובדתי לצד אירוע). `heartRate` הגולמי **אינו** מבוקש (אין צורך בדגימות פר-פעימה); heartbeat/RR ממתין ל-RRExtractor (Q-A). `NSHealthShareUsageDescription` + כל מסמכי ה-appstore מתואמים. אימות סופי מול Apple Review ב-🍎 |
